...
def get_parser():
    ...
    parser.add_argument(
        "--dataset-dir",
        type=str,
        default="download/CLAP_freesound/freesound_no_overlap",
    )
    
    parser.add_argument(
        "--manifest-dir",
        type=str,
        default="data/freesound_manifest",
    )
...
def main():
    ...
    if "train" in split:
        data_folder = f"{dataset_dir}/{args.split}/mnt/freesound/split/train"
    else:
        data_folder = f"{dataset_dir}/{args.split}/mnt/freesound/split/test"
    
    if truncate:
        manifest_output_dir = manifest_dir + "/" + f"freesound_cuts_{split}_{window_size}s.jsonl.gz"
    else:
        manifest_output_dir = manifest_dir + "/" + f"freesound_cuts_{split}.jsonl.gz"
    ...
    if os.path.exists(manifest_output_dir):
        ...
        audio_files = glob.glob(f"{data_folder}/*.flac", recursive=True)
        all_cuts = []
        num_cuts = 0
        for audio_file in audio_files:
            ...
            if truncate:
                ...
                meta_info = parse_meta(meta_file)
                for window_cut in cuts:
                    ...
                    sup.caption = meta_info["text"]
                    sup.tag = meta_info["tag"]
