...
class SVDataModule:
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            type=float,
            default=0.5,
            help="The probability of adding MUSAN noise to a cut",
        )
        ...
        group.add_argument(
            ...
            type=str2bool,
            default=True,
            help="When enabled, select noise from RIRs and do RIR augmentations"
        )
        ...
        group.add_argument(
            ...
            type=str,
            default="vox1",
            choices=["vox1", "vox2", "all"],
            help="Which subset of voxceleb to use.",
        )
    ...
    def train_dataloaders(
        ...
        if self.args.enable_musan:
            ...
            transforms.append(
                CutMix(cuts=cuts_musan, p=self.args.musan_prob, snr=(10, 20), preserve_id=True, pad_to_longest=False)
            )
        else:
    ...
    def voxceleb_cuts(self) -> CutSet:
        ...
        if self.args.voxceleb_subset == "vox2":
            ...
            cuts = load_manifest_lazy(
                self.args.manifest_dir / "vox2_cuts_dev.jsonl.gz"
            )
        elif self.args.voxceleb_subset == "vox1":
            cuts = load_manifest_lazy(
                self.args.manifest_dir / "vox1_cuts_dev.jsonl.gz"
            )
        elif self.args.voxceleb_subset == "all":
            logging.info("Get both the voxceleb1 and voxceleb2 cuts.")
            cuts1 = load_manifest_lazy(
                self.args.manifest_dir / "vox1_cuts_dev.jsonl.gz"
            )
            cuts2 = load_manifest_lazy(
                self.args.manifest_dir / "vox2_cuts_dev.jsonl.gz"
            )
            cuts = CutSet.mux(
                *[cuts1, cuts2],
    ...
    def voxceleb_cuts_3s(self) -> CutSet:
        ...
        if self.args.voxceleb_subset == "vox2":
            logging.info("Only get the voxceleb2 cuts.")
            cuts = load_manifest_lazy(
                self.args.manifest_dir / "vox2_cuts_dev_3s.jsonl.gz"
            )
        elif self.args.voxceleb_subset == "vox1":
            cuts = load_manifest_lazy(
                self.args.manifest_dir / "vox1_cuts_dev_3s.jsonl.gz"
            )
        elif self.args.voxceleb_subset == "all":
            logging.info("Get both the voxceleb1 and voxceleb2 cuts.")
            cuts1 = load_manifest_lazy(
    ...
    def voxceleb_dev_cuts(self) -> CutSet:
        logging.info("About to get the voxceleb dev cuts.")
        return load_manifest_lazy(
            self.args.manifest_dir / "vox1_cuts_dev.jsonl.gz"
        )
    ...
    def voxceleb_test_cuts(self) -> CutSet:
        logging.info("About to get the voxceleb test cuts.")
        return load_manifest_lazy(
            self.args.manifest_dir / "vox1_cuts_test.jsonl.gz"
        )
