...
def compute_loss(
    ...
    sv_targets = batch["labels"].to(device)
    
    if params.freeze_encoder_epochs > 0:
        freeze_encoder = params.cur_epoch <= params.freeze_encoder_epochs
        if random.random() < 0.01 and is_training:
            logging.info(f"Current epoch: {params.cur_epoch}. Freeze encoder: {freeze_encoder}")
    else:
    ...
    info = MetricsTracker(normalize=False)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        info["frames"] = (feature_lens // params.subsampling_factor).sum().item()
        info["utterances"] = feature.size(0)

    # Note: We already divided the loss by the batch size
    info["loss"] = loss.detach().cpu().item()
    info["sv_loss"] = loss.detach().cpu().item()
    return loss, info
...
def run(rank, world_size, args):
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        if c.duration < 0.5 or c.duration > 29.0:
            # logging.warning(
    ...
    train_dl = voxceleb.train_dataloaders(
        train_cuts,
        speaker_dict,
    ...
    # dev_cuts = voxceleb.voxceleb_dev_cuts()
    # valid_dl = voxceleb.valid_dataloaders(dev_cuts, speaker_dict=speaker_dict, world_size=world_size, rank=rank,)
    # valid_sets.append("vox1")
