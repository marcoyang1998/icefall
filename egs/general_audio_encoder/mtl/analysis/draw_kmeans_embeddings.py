...
def parse_args():
    ...
    parser.add_argument(
        "--manifest",
        type=str,
        default="hubert/embeddings/kmeans_hubert_base_layer_9_ls_dev-clean.jsonl.gz",
    )
...
def analyze_speaker_kmeans(args):
    ...
    # randomly get the first 10 speakers, otherwise visualisation is difficult
    offset = group * 10
    ...
    # get the kmeans centers
    km_centers = get_km_center("download/models/hubert_base_ls960_L9_km500.bin") # (C, D)
    km_centers = torch.from_numpy(km_centers) # (C, D)
    ...
    for i, cut in enumerate(cuts):
        ...
        labels.append(spkr)
        km_label = cut.kmeans # (T,C)
        km_label = torch.tensor(km_label) # (T)
        spkr_embed = km_centers[km_label].mean(dim=0, keepdim=True) # (1, C)
    ...
    fig = draw_umap(X_2d=all_embeddings_2d, labels=labels, show_legend=False)
    output_figure = f"analysis/figures/kmeans_embeddings/speaker_{suffix}.pdf"
    logging.info(f"Saving the figure to {output_figure}")
    fig.savefig(output_figure)

def get_km_center(km_path):
    import joblib
    km_model = joblib.load(km_path)
    centers = km_model.cluster_centers_ # (num_centers, dim)
    return centers
...
def analyze_audio_classification(args):
    ...
    output_figure = f"analysis/figures/kmeans_embeddings/esc_{suffix}.png"
    logging.info(f"Saving the figure to {output_figure}")
    fig = draw_umap(
        X_2d=all_embeddings_2d,
        labels=labels,
