...
def draw_reconstructed_embeddings_audio(args):
    ...
    fig = draw_umap(embed_2d, labels, label_mapping=class_mapping, show_legend=False)
...
def draw_reconstructed_embeddings_speaker(args):
    ...
    # speakers = [
    #     "id10270", "id10276", "id10278", "id10272", "id10282", "id10284", "id10291", "id10300",
    # ]
    ...
    reducer = umap.UMAP(random_state=42)
    embed_2d = reducer.fit_transform(all_embeddings)
    figure_name = f"analysis/figures/reconstructed_embeddings/{suffix}.pdf"
    logging.info(f"Saving the figure to {figure_name}")
    fig = draw_umap(embed_2d, labels, save_path=figure_name, show_legend=False)
...
def draw_reconstructed_embeddings_speaker_per_codebook(args):
    ...
    for n, codebook_embedding in enumerate(all_embeddings):
        ...
        folder = f"analysis/figures/reconstructed_embeddings_per_codebook/{suffix}"
        figure_name = f"{folder}/ls-dev-clean-cb-{n}.pdf"
        os.makedirs(folder, exist_ok=True)
        fig = draw_umap(embed_2d, labels, show_legend=False, save_path=figure_name)
...
if __name__=="__main__":
    ...
    if args.task == "speaker":
        draw_reconstructed_embeddings_speaker(args)
