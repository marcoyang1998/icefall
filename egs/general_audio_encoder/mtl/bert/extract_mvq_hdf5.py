...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Where to store the manifest augmented with bert features"
    )
...
def extract_embeddings(
    ...
    setup_logger(f"data/vq_bert_client/log/log-bert-cb-indexes")
    ...
    # currently only use the encoder of bert
    logging.info(params)
    model = MyBertModel(model_version=params.bert_version)
    model.eval()
    model.to(device)
    logging.info(f"Number of bert params: {sum(p.numel() for p in model.parameters())}")
    ...
    logging.info(f"Writing bert indexes")
    with h5py.File(embedding_path, "w") as writer:
        grp = writer.create_group("cb_indexes")
        for i, batch in enumerate(dl):
            ...
            for idx, cut in enumerate(cuts):
                grp.create_dataset(
                    cut.id,
                    data=codebook_indexes[idx][: embedding_lens[idx]],
                    compression="lzf"
                )
                cb_index_meta_info = {
                    "key": cut.id,
                ...
                new_cut = fastcopy(
                    cut,
                    custom={"codebook_indexes": cb_index_meta_info}
                )
    ...
    logging.info(f"Finished extracting bert codebook indexes, processed a total of {num_cuts} cuts.")
