...
from model_at import MultiTaskModel
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=1536,
        help="""Dimension used in the dasheng encoder model.
        """,
    )
    ...
    params = AttributeDict(
        {
            ...
            "feature_dim": 128,
            "subsampling_factor": 640,  # not passed in, this is fixed.
            "warm_step": 2000,
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    encoder = DashengEncoder(params.dasheng_version)
    return encoder
...
def compute_loss(
    ...
    audio = batch["audio"] # list of audio
    audio = [a.squeeze() for a in audio]
    audio = torch.nn.utils.rnn.pad_sequence(audio, batch_first=True).squeeze(1)
    audio = audio.to(device)
    audio_lens = torch.tensor(batch["audio_lens"]).to(device) # HuBERT is 50 Hz
    cuts = batch["cut"]
    
    at_targets = batch["at_targets"].to(device)
    ...
    with warnings.catch_warnings():
        ...
        info["utterances"] = len(cuts)

    # Note: We use reduction=sum while computing the loss.
    info["loss"] = loss.detach().cpu().item()
    info["audio_tagging_loss"] = loss.detach().cpu().item()
...
def display_and_save_batch(
    ...
    logging.info(f"features shape: {audio.shape}")
