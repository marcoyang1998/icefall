...
from model_multi_kd_mae import LinearSpectrogramTarget_1024, LinearSpectrogramTarget_Unnorm
...
class MultiKDModel(nn.Module):
    def __init__(
        ...
        num_events: int = 527,
        flow_target: str = "mel",
        flow_target_dim: int = 128,
        flow_downsample_factor: int = 4,
        flow_t_distribution: Callable = None,
        mask_mode: str = "w2v2",
        ...
        if self.flow_target == "linear":
            self.flow_target_extractor = LinearSpectrogramTarget_1024()
            # self.flow_target_extractor = LinearSpectrogramTarget_Unnorm()
            self.normalizing_stats = {"mean": LINEAR_SPECTROGRAM_MEAN, "var": LINEAR_SPECTROGRAM_VAR}
        else:
            self.normalizing_stats = {"mean": FBANK_MEAN, "var": FBANK_VAR}
            
        # sampling distribution of t in flow matching
        self.flow_t_distribution = flow_t_distribution
        
        self.flow_target_dim = flow_target_dim
        self.flow_downsample_factor = flow_downsample_factor
        
        self.decoder_embed = nn.Linear(encoder_dim, decoder_input_dim) # projecting encoder_out to decoder dim
        self.decoder_pred = nn.Linear(
            decoder_dim,
            flow_target_dim * flow_downsample_factor
        ...
        self.encoder_out_proj = nn.Linear(encoder_dim, encoder_dim, bias=True)
        self.decoder_in_proj = nn.Linear(encoder_dim + flow_target_dim * flow_downsample_factor, decoder_dim)
    ...
    def forward_flow_loss(
        self, encoder_out: torch.Tensor, encoder_out_lens: torch.Tensor, target: torch.Tensor, reduction: str = "none"
    ):
        # 1. 全局归一化
        target = (target - self.normalizing_stats["mean"]) / (self.normalizing_stats["var"] ** 0.5)  # (N, T, C)
        N, T, C = target.shape
        pad = (4 - (T % 4)) % 4
        if pad > 0:
            target = F.pad(target, (0, 0, 0, pad), mode="replicate")
        N, T, C = target.shape
        target_reshape = target.reshape(N, T//4, C * 4)  # (N, T_new, 4*C)
        target_reshape = self.truncate_target(encoder_out, target_reshape)
        
        # Get the time embedding
        if self.flow_t_distribution is not None:
            timestamps = self.flow_t_distribution(N).to(encoder_out.device) 
        else:
            timestamps = torch.rand(N, device=encoder_out.device)
        t_embed = timestep_embedding(timesteps=timestamps, dim=self.decoder_dim) 
        
        # flow target
        noise = torch.randn_like(target_reshape)
        x_t = (1 - timestamps[:, None, None]) * noise + timestamps[:, None, None] * target_reshape
        u_t = target_reshape - noise # (N,T,C), the flow training target 
        
        # flow input
        encoder_out = self.encoder_out_proj(encoder_out) # (N,T,C_enc)
        decoder_in = torch.cat([x_t, encoder_out], dim=-1) # (N,T,C_enc + target_dim)
        decoder_in = self.decoder_in_proj(decoder_in) + t_embed.unsqueeze(1) # (N, T, C_dec)
        ...
        decoder_out = self.decoder_pred(decoder_out) # (T,N,C_dec) -> (T,N, target_dim)
        decoder_out = decoder_out.permute(1, 0, 2) # (T, N, target_dim) -> (N, T, target_dim)

        loss = ((decoder_out - u_t)**2).sum(dim=-1) / target_reshape.size(-1)
        loss_mask = ~make_pad_mask(decoder_out_lens)
        if reduction == "sum":
            loss = (loss * loss_mask).sum(dim=-1) # (N)
        else:
            loss = loss * loss_mask
    ...
    def inference_flow(
        ...
        """Reconstruct the target (e.g. mel or linear spectrogram) from encoder
        outputs using Euler integration of the learned conditional vector field.

        The training objective follows conditional flow matching with a linear
        interpolation path:
            x_t = (1 - t) * x_0 + t * x_1,   x_0 ~ N(0,I),  x_1 = target
        so the ground-truth velocity is u_t = x_1 - x_0, and the network is
        trained to predict this velocity.  At inference we therefore integrate:
            x_{t + dt} = x_t + dt * v_theta(x_t, t, encoder_out)
        from t=0 to t=1 using K uniform Euler steps.

        Args:
            encoder_out (torch.Tensor): Encoder output of shape (N, T, C_enc).
            encoder_out_lens (torch.Tensor): Valid lengths, shape (N,).
            K (int): Number of uniform Euler integration steps (used if schedule is None).
            schedule (list): A list of floats specifying the integration path, e.g.
                [0.0, 0.05, 0.1, 0.2, 0.5, 1.0]. Must start with 0.0 and end with 1.0.

        Returns:
            pred (torch.Tensor): Reconstructed frames in the **normalised**
                feature space, shape (N, T, flow_target_dim).
            decoder_out_lens (torch.Tensor): Valid lengths of the output,
                shape (N,).
        """
        N, T, _ = encoder_out.shape

        if schedule is not None:
            assert math.isclose(schedule[0], 0.0, abs_tol=1e-5), f"Schedule must start at 0.0, got {schedule[0]}"
            assert math.isclose(schedule[-1], 1.0, abs_tol=1e-5), f"Schedule must end at 1.0, got {schedule[-1]}"
            steps = len(schedule) - 1
            t_schedule = schedule
        else:
            steps = K
            t_schedule = [k / K for k in range(K + 1)]

        # Start from pure Gaussian noise in the (T, flow_target_dim * flow_downsample_factor) space
        x_t = torch.randn(
            N, T, self.flow_target_dim * self.flow_downsample_factor,
            device=encoder_out.device,
            dtype=encoder_out.dtype,
        )

        # Project encoder features once (shared across all steps)
        enc_proj = self.encoder_out_proj(encoder_out)  # (N, T, C_enc)
        ...
        for k in range(steps):
            t = t_schedule[k]
            dt = t_schedule[k+1] - t
            timestamps = torch.full((N,), t, device=encoder_out.device, dtype=encoder_out.dtype)
            t_embed = timestep_embedding(timesteps=timestamps, dim=self.decoder_dim)  # (N, C_dec)

            # Build decoder input
            decoder_in = torch.cat([x_t, enc_proj], dim=-1)          # (N, T, C_enc + target_dim*ds)
            decoder_in = self.decoder_in_proj(decoder_in) + t_embed.unsqueeze(1)  # (N, T, C_dec)

            decoder_in = decoder_in.permute(1, 0, 2)  # (T, N, C_dec)
            decoder_out, decoder_out_lens = self.decoder(
                x=decoder_in,
                x_lens=encoder_out_lens,
                src_key_padding_mask=padding_mask,
            ...
            # Predicted velocity: (T, N, target_dim*ds) -> (N, T, target_dim*ds)
            v_t = self.decoder_pred(decoder_out).permute(1, 0, 2)

            # Euler step
            x_t = x_t + dt * v_t

        # x_t is now shape (N, T, flow_target_dim * flow_downsample_factor)
        # Reshape to (N, T * flow_downsample_factor, flow_target_dim) to recover per-frame features
        x_t = x_t.reshape(N, T * self.flow_downsample_factor, self.flow_target_dim)

        # De-normalise back to original feature scale
        mean = self.normalizing_stats["mean"]
        std = self.normalizing_stats["var"] ** 0.5
        pred = x_t * std + mean

        return pred, decoder_out_lens * self.flow_downsample_factor
    ...
    def forward_mae_loss(
        ...
        padding_mask = ~ make_pad_mask(decoder_out_lens * self.flow_downsample_factor)
    ...
    def forward(
        ...
        flow_loss = self.forward_flow_loss(encoder_out, encoder_out_lens, flow_target, reduction="none")
        
        # For flow: 
        # We only compute the fbank reconstruction loss on masked positions, regardless of loss_only_mask
        if mask_indices is not None:
            ds_mask_indices = self.downsample_mask_indices(mask_indices)
        
            diff = ds_mask_indices.shape[1] - flow_loss.shape[1]
            if diff > 0:
                ds_mask_indices = ds_mask_indices[:, diff//2: diff//2 + flow_loss.shape[1]]
            flow_loss = flow_loss * ds_mask_indices
        flow_loss = flow_loss.sum(dim=-1) # (N)
        
        return codebook_loss, at_loss, flow_loss
    ...
    def downsample_mask_indices(self, mask: torch.Tensor, pooling_type: str="avg"):
        stride = self.flow_downsample_factor
        assert stride in [2,4], "Only support downsampling factor of 2 or 4"
        if pooling_type == "avg":
            ...
            mask = nn.functional.max_pool1d(mask, stride)
        return mask
