...
def main():
    ...
    if not params.use_averaged_model:
        ...
        if params.iter > 0:
            if params.use_s3_client:
                filename_start = f"{params.s3_prefix}/{params.exp_dir}/checkpoint-{params.iter}.pt"
                end_iter = params.iter - 4000 * params.avg
                filename_end = f"{params.s3_prefix}/{params.exp_dir}/checkpoint-{end_iter}.pt"
            else:
                filenames = find_checkpoints(params.exp_dir, iteration=-params.iter)[
                    : params.avg + 1
                ]
