...
class MultiKDModel(nn.Module):
    def __init__(
        ...
        flow_weighted_sum: bool = False,
        flow_weighted_sum_normalize: bool = False,
        flow_temperature: float = 1.0,
        flow_topk_layers: int = -1,
        mask_mode: str = "w2v2",
        ...
        Args:
          ...
          flow_weighted_sum:
            If use weighted sum of encoder layers as input to the flow decoder
          mask_prob:
        ...
        # weighted sum of encoder features to decoder
        self.flow_weighted_sum = flow_weighted_sum
        self.flow_topk_layers = flow_topk_layers
        self.flow_weighted_sum_normalize = flow_weighted_sum_normalize
        self.flow_temperature = flow_temperature
        if self.flow_weighted_sum:
            if flow_topk_layers > 0:
                zero_init = torch.cat([torch.zeros(flow_topk_layers)])
    ...
    def forward_encoder(
        self, x: torch.Tensor, x_lens: torch.Tensor, return_weighted_out: bool = False,
        ...
        assert weighted_sum_out.shape == encoder_out.shape, f"weighted_sum_out shape {weighted_sum_out.shape} does not match encoder_out shape {encoder_out.shape}"

        return encoder_out, encoder_out_lens, weighted_sum_out
    ...
    def forward(
        ...
        encoder_out, encoder_out_lens, weighted_sum_out = self.forward_encoder(
            x,
            x_lens,
            return_weighted_out=self.flow_weighted_sum,
        )
        ...
        if self.flow_weighted_sum:
            assert weighted_sum_out is not None, "weighted_out is None but flow_weighted_sum is True"
            flow_loss = self.forward_flow_loss(weighted_sum_out, encoder_out_lens, flow_target, reduction="none")
        else:
