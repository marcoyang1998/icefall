...
from model_multi_kd_flow_weighted_sum import MultiKDModel
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="The temperature for computing layer-wise weights"
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If we use the weighted sum of encoder features as the condition for flow matching"
        "Note that we will still use the last layer for predicting the MVQ tokens"
    )
    
    parser.add_argument(
        "--flow-weighted-sum-normalize",
        type=str2bool,
        default=False,
        help="If True, apply layer normalization to the weighted sum of encoder features"
        "before feeding to the flow decoder"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Use the weighted sum of topk layers of encoder as condition for the decoder"
        "If -1, use all layers."
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = MultiKDModel(
        ...
        flow_weighted_sum=params.flow_weighted_sum,
        flow_weighted_sum_normalize=params.flow_weighted_sum_normalize,
        flow_temperature=params.flow_temperature,
        flow_topk_layers=params.flow_topk_layers,
