...
from dataset3 import MultiTaskKDDataset
...
class MultiTaskDataModule:
    ...
    def audioset_cuts(self) -> CutSet:
        ...
        if self.args.audioset_subset == "full":
            if not self.args.at_weighted_sampler:
                if self.args.use_shar:
                    # all_cuts = sorted(glob.glob("data-shar-no-feat/audioset/full/cuts.*.jsonl.gz"))
                    # all_recordings = sorted(glob.glob("data-shar-no-feat/audioset/full/recording.*.tar"))
                    # all_beats_embed = sorted(glob.glob("data-shar-no-feat/audioset/full/beats_embedding.*.tar"))
                    # all_cb_indexes = sorted(glob.glob("data-shar-audioset-cb/audioset/full/audio_codebook_indexes.*.tar"))
                    cuts = CutSet.from_shar(
                        in_dir="data-shar-audioset-cb/audioset/full",
                        shuffle_shards=True,
                        stateful_shuffle=True,
                        seed="randomized",
                    ).repeat()
    ...
    def audioset_eval_cuts(self) -> CutSet:
        logging.info("About to get test-other cuts")
        return load_manifest_lazy(
            "data/vq_dasheng_large_layer_-1_cb16/audioset_cuts_eval.jsonl.gz"
        )
