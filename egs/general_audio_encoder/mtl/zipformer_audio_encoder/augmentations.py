...
class BatchMixingWithTask:
    def __init__(
        ...
        The mechanism is the same as BatchMixing, but we apply different augmentations to data from
        different tasks. 
        For Speech:
            Total augmentation prob p, among which p_noise probability to mix with noise, otherwise mix
            with other speech data
        For Non-Speech:
            Only mix with noise with probability p_noise
        ...
        Args:
            ...
            p_noise (float, optional): The probability of performing noise mixing instead of in-batch (for speech task data).
            p (float, optional): The probability of performing mixing to a cut. Defaults to 0.5 (for speech task data).
    ...
    def __call__(self, reference_cuts: CutSet) -> CutSet:
        ...
        # helper function to resolve attributes repetition issue
        def remove_custom(c):
            c.custom = None
            return c
        ...
        for cut in reference_cuts:
            ...
            if cut.task_id == 1:
                ...
                raise ValueError(f"Unsupported task id {cut.task_id}")
...
def _test_mix():
    ...
    print(f"Noise mix: {num_noise_mix}; Speech mix: {num_speech_mix}")
...
def _test_mix_with_task():
    from functools import partial
    from lhotse import load_manifest_lazy, load_manifest
    from utils import _add_task_id
    
    speech_cuts = load_manifest_lazy("data/fbank/librispeech_cuts_dev-other.jsonl.gz").drop_features().subset(first=200)
    audio_cuts = load_manifest_lazy("data/fbank_as_ced_mAP50/audioset_cuts_balanced.jsonl.gz").drop_features().subset(first=200)
    noise_cuts = load_manifest("data/musan/noise_non_speech_musan_audioset.jsonl.gz").drop_features()
    
    speech_cuts = speech_cuts.map(partial(_add_task_id, 1))
    audio_cuts = audio_cuts.map(partial(_add_task_id, 2))
    
    cuts = CutSet.mux(
        *[speech_cuts, audio_cuts],
        weights=[1,1],
        stop_early=False,
    )
    ...
    for i in range(5):
        ...
        for j, c in enumerate(mixed_cuts):
            if isinstance(c, MixedCut):
                if c.tracks[0].cut.task_id == 1:
                    num_speech_mix += 1
                elif c.tracks[0].cut.task_id == 2:
                    num_audio_mix += 1
                else:
                    raise ValueError(f"Unsupported task id {c.tracks[0].task_id}")
        print(f"A total of {len(mixed_cuts)} cuts. Number of augmented speech cuts: {num_speech_mix}; Number of augmented audio cuts: {num_audio_mix}")
