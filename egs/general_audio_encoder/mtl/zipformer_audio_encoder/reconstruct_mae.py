...
from train_multi_KD3_shar_mae import add_model_arguments, get_model, get_params
...
def reconstruct_feature(
    ...
    encoder_out, encoder_out_lens = model.forward_encoder(feature, feature_lens) # (B,T,C)
    
    # the predition if already converted back to the original scale
    decoder_out, decoder_out_lens = model.forward_decoder(encoder_out, encoder_out_lens)
    pred = model.decoder_pred(decoder_out)
    pred = pred.reshape(pred.size(0), -1, reconstruction_target_dim)
    
    if reconstruction_target_type == "mel":
        target = feature
    elif reconstruction_target_type == "linear":
        target = model.mae_target_extractor(audio)
...
def main():
    ...
    for i, cut in enumerate(cuts):
        pred, target = reconstruct_feature(
            ...
            fbank_extractor,
            cut,
            reconstruction_target_type=params.mae_target,
            reconstruction_target_dim=params.mae_target_dim,
        )
