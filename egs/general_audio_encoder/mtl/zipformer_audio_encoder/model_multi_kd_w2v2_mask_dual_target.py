...
class MultiKDModel(nn.Module):
    ...
    def forward(
        ...
        if codebook_indexes is not None and self.codebook_loss_net is not None:
            for cur_cb_indexes in codebook_indexes:
                codebook_loss = self.forward_codebook_loss(
                    encoder_out, encoder_out_lens, cur_cb_indexes, reduction="none"
                )
                if self.loss_only_mask and mask_indices is not None:
                    # downsample the mask 
                    cur_mask_indices = nn.functional.avg_pool1d(mask_indices, 2) >= 0.5
                    assert cur_mask_indices.size(1) >= codebook_loss.size(1)
                    cur_mask_indices = cur_mask_indices[:, :codebook_loss.size(1)].float()
        ...
        return codebook_losses, at_loss
