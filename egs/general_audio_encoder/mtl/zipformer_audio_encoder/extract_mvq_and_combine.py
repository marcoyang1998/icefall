...
def extract_embeddings(
    ...
    if params.output_downsampling_factor == 2:
        assert params.frame_shift == 0.04
    elif params.output_downsampling_factor == 1:
        assert params.frame_shift == 0.02
    else:
        raise ValueError(f"Unsupported output downsampling factor: {params.output_downsampling_factor}")
...
def change_yodas_recording_root(c):
    source = c.recording.sources[0].source
    source = source.replace(
