...
def run(rank, world_size, args):
    ...
    en_asr_training_cuts = en_asr_training_cuts.map(normalize_english_text)
    ...
    # normalize the chinese text and convert to bytes
    zh_asr_training_cuts = zh_asr_training_cuts.map(normalize_chinese_text)
    ...
    asr_training_cuts = asr_training_cuts.map(tokenize_and_encode_text)
    ...
    if params.use_gigaspeech:
        ...
        giga_dev_cuts = giga_dev_cuts.map(tokenize_and_encode_text)
    ...
    if params.use_wenetspeech:
        ...
        wenet_dev_cuts = wenet_dev_cuts.map(tokenize_and_encode_text)
    ...
    if params.use_aishell:
        ...
        aishell_dev_cuts = aishell_dev_cuts.map(tokenize_and_encode_text)
