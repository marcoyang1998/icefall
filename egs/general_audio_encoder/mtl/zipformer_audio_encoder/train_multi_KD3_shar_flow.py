...
from model_multi_kd_flow import MultiKDModel
...
from zipformer2_layerwise import Zipformer2
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    # flow decoder related arguments
    parser.add_argument(
        "--num-decoder-layers",
        type=str,
        default="2,2,2,2,2,2",
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If True, use the output of decoder for MVQ loss. Otherwise use the output of encoder for MVQ loss."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="The dimension of the flow target (frame level)"
    )
    
    parser.add_argument(
        "--flow-downsample-factor",
        type=int,
        default=4,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="The loss scale for flow loss"
    )
    
    # flow distribution related
    parser.add_argument(
        "--flow-t-distribution",
        type=str,
        default="static",
        choices=["static", "uniform", "beta"],
        help="Which distribution of t to use"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Only used in static distribution, the value of t."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Only used in Beta distribution"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Only used in uniform distribution, the max value of t"
    )
...
def get_t_distribution(distribution_type: str, params) -> Callable[[int], torch.Tensor]:
    """
    Return a sampler function f(N) -> Tensor[N] for t sampling.

    Supported distributions:
      - "uniform": U(0, 1)
      - "beta": Beta(params.t_alpha, params.t_beta)
      - "static": Static(params.t_value)
    """
    dist_name = distribution_type.lower().strip()

    if dist_name == "uniform":
        def _sample_uniform(N: int) -> torch.Tensor:
            return torch.rand(N, dtype=torch.float32) * params.flow_t_max
        logging.info(f"Using uniform distribution for flow t with max value={params.flow_t_max}")
        return _sample_uniform

    if dist_name == "beta":
        alpha = float(params.flow_t_alpha)
        beta = float(params.flow_t_beta)
        if alpha <= 0 or beta <= 0:
            raise ValueError(
                f"Beta parameters must be > 0, got t_alpha={alpha}, t_beta={beta}"
            )
        ...
        def _sample_beta(N: int) -> torch.Tensor:
            if N <= 0:
                return torch.empty(0, dtype=torch.float32)
            return beta_dist.sample((N,)).to(torch.float32)
        logging.info(f"Using Beta distribution for flow t with alpha={alpha}, beta={beta}")
        return _sample_beta
    
    if dist_name == "static":
        def _sample_static(N: int) -> torch.Tensor:
            return torch.full((N,), fill_value=params.flow_t_value, dtype=torch.float32)
        logging.info(f"Using static distribution for flow t with value={params.flow_t_value}")
        return _sample_static

    raise ValueError(
        f"Unsupported t distribution: '{distribution_type}'. "
        "Use one of: ['uniform', 'beta', 'static']"
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    assert params.flow_downsample_factor == params.output_downsampling_factor * 2
    
    if params.loss_only_mask:
        logging.info("Only computing loss on the masked positions")
        
    # determine the distribution of t in flow matching
    flow_t_distribution = get_t_distribution(params.flow_t_distribution, params)
    
    if params.decoder_mvq:
        from model_multi_kd_flow_decoder_mvq import MultiKDModel
        logging.info("Using the decoder for MVQ token prediction")
    else:
        from model_multi_kd_flow import MultiKDModel
        logging.info("Using the encoder for MVQ token prediction")
    ...
    model = MultiKDModel(
        ...
        flow_target_dim=params.flow_target_dim,
        flow_downsample_factor=params.flow_downsample_factor,
        flow_t_distribution=flow_t_distribution,
...
def compute_loss(
    ...
    if params.flow_target == "linear":
        if isinstance(model, DDP):
            flow_target = model.module.flow_target_extractor(audio) # (N,T, feature_dim)
        else:
            flow_target = model.flow_target_extractor(audio) # (N,T, feature_dim)
    else:
        flow_target = feature.clone()
    ...
    with torch.set_grad_enabled(is_training):
        mvq_loss, audio_tagging_loss, flow_loss = model(
            x=feature,
            x_lens=feature_lens,
    ...
    info["flow_loss"] = flow_loss.detach().cpu().item()
...
def run(rank, world_size, args):
    ...
    logging.info(f"Using {params.flow_target} spectrogram as the flow target")
    ...
    if params.use_soundnet:
        ...
        soundnet_cuts = librispeech.soundnet_raw_train_cuts()
    ...
    if params.use_acavcaps:
        logging.info(f"Getting ACAVCaps cuts")
        acavcaps_cuts = librispeech.acavcaps_raw_all_cuts()
        acavcaps_cuts = acavcaps_cuts.map(partial(_add_task_id, 2))
        audio_training_cuts.append(acavcaps_cuts)
        acavcaps_cuts_lens = 1888837  # 1888837 cuts
        acavcaps_cuts_duration = 5252  # 5252 hours
        audio_training_cuts_lens.append(acavcaps_cuts_lens)
        audio_training_cuts_duration.append(acavcaps_cuts_duration)
