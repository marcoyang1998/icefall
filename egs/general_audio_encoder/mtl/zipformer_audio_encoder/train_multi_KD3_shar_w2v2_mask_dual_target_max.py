...
from kd_datamodule3_shar_dual_target_max import MultiTaskDataModule
from lhotse import CutSet
from lhotse.cut import Cut, MonoCut
from lhotse.dataset.sampling.base import CutSampler
from lhotse.utils import fix_random_seed
from model_multi_kd_w2v2_mask_dual_target import MultiKDModel
...
def compute_loss(
    ...
    # batch_mix_cuts = 0
    # for c in cuts:
    #     if getattr(c, "mix_type", "none") == "speech":
    #         # logging.info(
    #         #     "Orig duration: {:.2f}, mixed duration: {:.2f}, total duration: {:.2f}".format(
    #         #         c.tracks[0].cut.duration,
    #         #         c.tracks[1].cut.duration,
    #         #         c.duration,
    #         #     )
    #         # )
    #         batch_mix_cuts += 1
    # if is_training:
    #     logging.info(f"Number of mixed cuts in the batch: {batch_mix_cuts}/{len(cuts)}")
    
        
    feature_lens = supervisions["num_frames"].to(device)
    if feature_lens.max() > feature.size(1):
        assert feature_lens.max() - feature.size(1) <= 2, (feature_lens.max(), feature.size(1))
        feature_lens = torch.clamp_max(feature_lens, feature.size(1))
    ...
    with torch.set_grad_enabled(is_training):
        mvq_losses, audio_tagging_loss = model(
            x=feature,
            x_lens=feature_lens,
            codebook_indexes=[mvq_tokens, mixed_cb_indexes],
