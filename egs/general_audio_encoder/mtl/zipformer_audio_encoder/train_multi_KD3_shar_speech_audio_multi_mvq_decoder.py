...
from model_multi_kd_multi_teacher_decoder import MultiKDModel
from mvq_decoder import MvqDecoder
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="""Number of decoder layers for MVQ token prediction. Default value is 0, meaning
        the MVQ prediction directly follows the encoder.
        """
    )
...
def get_decoder(params: AttributeDict) -> nn.Module:
    ...
    for (num_cb, frame_ratio, delta) in zip(num_codebooks, teacher_frame_ratio, distillation_delta):
        decoder = MvqDecoder(
            encoder_dim=encoder_dim,
            frame_ratio=frame_ratio,
            delta=delta,
            num_codebooks=num_cb,
            output_downsampling_factor=1, # this is fixed
            num_decoder_layers=_to_int_tuple(params.num_decoder_layers),
            downsampling_factor=_to_int_tuple(params.decoder_downsampling_factor),
            decoder_dim=_to_int_tuple(params.decoder_dim),
            decoder_unmasked_dim=_to_int_tuple(params.decoder_unmasked_dim),
            num_heads=(4,4)
        )
        logging.info(sum([p.numel() for p in decoder.parameters()]))
        logging.info(sum([p.numel() for p in decoder.predictor.parameters()]))
