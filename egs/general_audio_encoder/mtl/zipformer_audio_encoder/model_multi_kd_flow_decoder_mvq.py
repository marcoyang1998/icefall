...
class MultiKDModel(nn.Module):
    def __init__(
        ...
        # In this version, we put the prediction of the MVQ tokens at the decoder side
        if num_codebooks > 0:
            self.codebook_loss_net = JointCodebookLoss(
                predictor_channels=decoder_dim,
    ...
    def forward_flow_loss(
        ...
        final_decoder_out = self.decoder_pred(decoder_out) # (T,N,C_dec) -> (T,N, target_dim)
        final_decoder_out = final_decoder_out.permute(1, 0, 2) # (T, N, target_dim) -> (N, T, target_dim)

        loss = ((final_decoder_out - u_t)**2).sum(dim=-1) / target_reshape.size(-1)
        ...
        return loss, decoder_out, decoder_out_lens
    ...
    def forward(
        ...
        flow_loss, decoder_out, decoder_out_lens = self.forward_flow_loss(encoder_out, encoder_out_lens, flow_target, reduction="none")
        ...
        if codebook_indexes is not None and self.codebook_loss_net is not None:
            codebook_loss = self.forward_codebook_loss(
                decoder_out, decoder_out_lens, codebook_indexes, reduction="none"
            )
