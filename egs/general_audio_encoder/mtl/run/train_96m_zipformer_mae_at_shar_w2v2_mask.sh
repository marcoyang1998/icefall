...
out-ds-${output_downsampling_factor}-\
${mask_mode}-mask-prob-${mask_prob}-len-${mask_length}-\
...
small-decoder-dasheng-large-as-mvq-cb8-musan-${enable_musan}-shar
...
torchrun \
  --nproc_per_node 8 \
  zipformer_audio_encoder/train_multi_KD3_shar_mae_w2v2_mask.py \
    --num-epochs 2 \
    --start-epoch 1 \
    --max-iter 300000 \
