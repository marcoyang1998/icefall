...
exp_dir=zipformer_audio_encoder/exp-xlarge-lr-${lr}-causal-ls-giga-${gigaspeech_subset}-wenet-${wenetspeech_subset}-\
-mls-${use_mls}-at-${audioset_subset}-multi-mvq-kd-separate-mvq-${separate_mvq}-md-${max_duration}
...
torchrun \
  --nproc_per_node 6 --nnodes 1 --node_rank $SLURM_PROCID --master_addr $master_addr\
