...
export PYTHONPATH=/mnt/cache/housiyuan/workspace/icefall_audio_encoder:$PYTHONPATH
...
# -mls-${use_mls}-extra-zh-en-as-${audioset_subset}-multi-mvq-kd-cb-${num_codebooks}\
# -at-kd-scale-${audio_tagging_loss_scale}-md-${max_duration}
...
torchrun \
  --nproc_per_node 1 \
  zipformer_audio_encoder/train_multi_KD3_shar_multi_mvq2.py \
    --num-epochs 2 \
    --start-epoch 1 \
    --max-iters 500000 \
    --use-shar $use_shar \
