...
vq_dir=data/vq_hubert_${model_version}_layer_${embedding_layer}_normalize_${normalize}_cb_${num_codebooks}
mkdir -p $vq_dir

quantizer_path=data/quantizer/hubert-${model_version}-layer-${embedding_layer}-normalize-${normalize}-cb-${num_codebooks}.pt

prefix_folder=/cpfs02/user/housiyuan/xiaoyu/codebook_indexes/hubert_${model_version}_layer_${embedding_layer}_normalized_cb_${num_codebooks}
...
if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
    ...
    python hubert/extract_mvq.py \
        ...
        --s3-prefix $prefix_folder/vox1-test \
        --embedding-dir $vq_dir \
        --embedding-layer $embedding_layer \
        --quantizer-path $quantizer_path \
        --normalize $normalize \
...
if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
    ...
    for subset in Train Test1 Test2 Development; do
        python hubert/extract_mvq.py \
            ...
            --s3-prefix $prefix_folder/msp-${subset} \
            --embedding-dir $vq_dir \
            --embedding-layer $embedding_layer \
            --quantizer-path $quantizer_path \
            --normalize $normalize \
...
if [ $stage -le 10 ] && [ $stop_stage -ge 10 ]; then
    ...
    for subset in all; do
        python hubert/extract_mvq.py \
            ...
            --s3-prefix $prefix_folder/iemocap-${subset} \
            --embedding-dir $vq_dir \
            --embedding-layer $embedding_layer \
            --quantizer-path $quantizer_path \
            --normalize $normalize \
...
if [ $stage -le 11 ] && [ $stop_stage -ge 11 ]; then
    ...
    for subset in train dev test; do
        python hubert/extract_mvq.py \
            ...
            --s3-prefix $prefix_folder/meld-${subset} \
            --embedding-dir $vq_dir \
            --embedding-layer $embedding_layer \
            --quantizer-path $quantizer_path \
            --normalize $normalize \
...
if [ $stage -le 12 ] && [ $stop_stage -ge 12 ]; then
    ...
    for subset in all; do
        python hubert/extract_mvq.py \
            ...
            --s3-prefix $prefix_folder/mead-${subset} \
            --embedding-dir $vq_dir \
            --embedding-layer $embedding_layer \
            --quantizer-path $quantizer_path \
            --normalize $normalize \
