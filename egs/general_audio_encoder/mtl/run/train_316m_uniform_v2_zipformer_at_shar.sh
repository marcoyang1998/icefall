...
exp_dir=zipformer_audio_encoder/exp-316M-uniform-v2-zipformer-lr-hours-${lr_hours}-\
as-${audioset_subset}-vgg-bbc-m4a-fs-mtg-\
${mask_mode}-mask-prob-${mask_prob}-mask-len-${mask_length}-\
channel-mask-prob-${mask_channel_prob}-len-${mask_channel_length}-\
dasheng-mvq-cb8-musan-mix-shar
...
torchrun --nproc_per_node=8 --master_port=19132 \
  zipformer_audio_encoder/train_multi_KD3_shar_w2v2_mask.py \
    ...
    --start-epoch 1 \
    --max-iter 300000 \
    --use-shar $use_shar --shar-dir data-shar/data-shar-dasheng-as-cb8 \
    --manifest-dir data/vq_dasheng_large_layer_-1_normalize_0_cb_8 \
