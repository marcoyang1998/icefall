...
# finetune args
do_finetune=1
finetune_ckpt=transformer/exp-transformer-causal-0-adam-lr-5e-4-warmup-25000-ls-full-mask-ratio-1.0-musan-1-rir-0-hubert-large-layer-21-libri-mvq-cb16-shar/iter-92000-avg-2.pt
...
exp_dir=transformer/exp-finetune-as-${audioset_subset}\
...
torchrun --nproc_per_node=1 --master_port=19130 \
  transformer/finetune_at.py \
    --num-epochs 30 \
    --start-epoch 1 \
    --use-fp16 1 \
