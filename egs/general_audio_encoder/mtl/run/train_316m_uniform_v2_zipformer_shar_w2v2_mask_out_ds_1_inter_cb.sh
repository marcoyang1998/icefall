...
# intermediate mvq KD
intermediate_cb=1
intermediate_block_idx=4
intermediate_cb_loss_scale=0.5
...
exp_dir=zipformer_audio_encoder/exp-316M-uniform-v2-zipformer-out-ds-${output_downsampling_factor}-\
lh-${libriheavy_subset}-giga-${gigaspeech_subset}-vox-${use_voxpopuli}-\
lr-hours-${lr_hours}-lr-batches-${lr_batches}-${mask_mode}-p-${mask_prob}-l-${mask_length}-\
cha-p-${mask_channel_prob}-l-${mask_channel_length}-\
inter-cb-${intermediate_cb}-block-${intermediate_block_idx}-scale-${intermediate_cb_loss_scale}-\
${mixing_mode}-mix-p-${mixing_prob}-min-snr-${min_snr}-p-noise-${p_noise}-min-snr-${min_noise_snr}-\
wavlm-large-layer-21-cb-${num_codebooks}-shar-md-${max_duration}
...
torchrun --nproc_per_node=8 \
  ...
  zipformer_audio_encoder/train_multi_KD3_shar_inter_cb_w2v2_mask.py \
    --num-epochs 2 \
    --start-epoch 1 \
    --max-iter 500000 \
    ...
    --intermediate-cb $intermediate_cb --intermediate-block-idx $intermediate_block_idx --intermediate-cb-loss-scale $intermediate_cb_loss_scale \
