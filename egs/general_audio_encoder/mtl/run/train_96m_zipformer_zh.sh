...
exp_dir=zipformer_audio_encoder/exp-wenetspech-L-96M-zipformer-non-streaming-\
whisper-mvq-v2-firered-mvq-en-zh-cb16-out-ds-${output_downsampling_factor}-\
mask-ratio-${time_mask_ratio}-musan-${enable_musan}
# exp_dir=zipformer_audio_encoder/exp-wenetspech-L-96M-zipformer-non-streaming-firered-mvq-cb16-out-ds-2-mask-ratio-1.0-musan-0
...
torchrun \
  ...
  zipformer_audio_encoder/train_multi_KD3_shar.py \
    ...
    --start-epoch 1 \
    --max-iter 204000 \
    --use-shar $use_shar --shar-dir data-shar-whisper-cb16-firered-cb16-no-feat \
    --base-lr $lr \
    --use-fp16 1 \
    --exp-dir $exp_dir \
    ...
    --causal 0 \
    --manifest-dir data/vq_whisper_turbo_cb16_firered_zh_en_cb16 \
