...
def chunk_forward(
    ...
    while True:
        ...
        # 1. compute fbank for the current chunk, beside the frames within the current chunk, we also nedd
        #       a. Some trailing fbank frames, covered by the convolution kernels in the encoder_embed
        #       b. Some extra tolerance frames, to give precise last fbank frame (the tolorance fbank frame will be removed )
        
        audio_chunk = audio[:, num_processed_samples: num_processed_samples + (chunk_size_samples + pad_length_samples + extra_tolerance_samples)] # (num_samples)
        
        features, _  = compute_fbank(audio_chunk, torch.tensor([audio_chunk.shape[-1]])) # shape: (T, num_mels)
        features_all.append(features[0, :2*chunk_size])
    ...
    return encoder_outs, encoder_out_lens, features_all, layerwise_outs
...
def main(args):
    ...
    import pdb; pdb.set_trace()
    encoder_out, encoder_out_lens, input_fbank, intermediate_hidden_states = chunk_forward(
