...
def extract_embeddings(
    ...
    setup_logger(f"data/embeddings/log/log-wavlm-embeddings")
    ...
    model.to(device)
    model.eval()
    logging.info(f"Number of wavlm model params: {sum(p.numel() for p in model.parameters())}")
    ...
    with NumpyHdf5Writer(embedding_path) as writer:
        logging.info(f"Writing wavlm embeddings to {embedding_path}")
    ...
    logging.info(f"Finished extracting wavlm embeddings, processed a total of {num_cuts} cuts.")
