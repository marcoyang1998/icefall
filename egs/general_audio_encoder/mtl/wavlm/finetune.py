...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If add a layernorm after the WavLM encoder,"
    )
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    logging.info(f"Loading from the pre-trained WavLM model")
    encoder = WavlmModel(
...
def save_checkpoint(
    ...
    if not params.freeze_encoder:
        ...
        model_state_dict = {k: state_dict[k] for k in state_dict if "encoder." not in k } # remove the encoder parameters
...
def run(rank, world_size, args):
    ...
    valid_cuts = valid_cuts.drop_features()
