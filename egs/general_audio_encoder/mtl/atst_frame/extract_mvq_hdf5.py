...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Where to store the manifest augmented with atst_frame features"
    )
...
def extract_embeddings(
    ...
    setup_logger(f"data/vq_atst_frame/log/log-atst_frame-cb-indexes")
    ...
    with NumpyHdf5Writer(embedding_path) as writer:
        logging.info(f"Writing atst_frame indexes to {embedding_path}")
        for i, batch in enumerate(dl):
            ...
            for idx, cut in enumerate(cuts):    
                ...
                new_cut = fastcopy(
                    cut,
                    custom={**cut.custom, "codebook_indexes": cb_index}
                )
    ...
    logging.info(f"Finished extracting atst_frame codebook indexes, processed a total of {num_cuts} cuts.")
