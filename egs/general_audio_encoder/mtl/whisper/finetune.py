...
from teachers import WhisperEncoder
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Which layer's feature to extract."
    )
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    logging.info(f"Loading from the pre-trained Whisper {params.whisper_version} model")
    model = whisper.load_model(params.whisper_version, device="cpu")
    model = model.encoder
    model.eval()
...
def compute_loss(
    ...
    feature_lens = batch["audio_lens"] # HuBERT is 50 Hz
...
def run(rank, world_size, args):
    ...
    train_dl = librispeech.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict, rank=rank, world_size=world_size,
    )

    valid_cuts = librispeech.dev_clean_cuts()
    valid_cuts += librispeech.dev_other_cuts()
    
    valid_cuts = valid_cuts.drop_features().filter(remove_short_and_long_utt)

    valid_sets = ["librispeech"]
    valid_dls = [
        librispeech.valid_dataloaders(valid_cuts, rank=rank, world_size=world_size,),
    ]
