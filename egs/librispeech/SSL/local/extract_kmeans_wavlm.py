...
def get_args():
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        default="models/WavLM-Base.pt",
    )

    parser.add_argument(
        "--kmeans-model-path",
        type=str,
        default="models/wavlm-Base-layer-9.kmeans.500.model",
    )
...
def extract_kmeans(args):
    ...
    assert manifest_dir.exists(), f"{manifest_dir} does not exist!"
    ...
    if args.num_jobs == 1:
        ...
        raw_cuts_path = manifest_dir / f"{prefix}_cuts_{args.subset}.jsonl.gz"
