...
dl_dir=$PWD/download
fbank_dir=data/fbank_as_ced_mAP50
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  ...
  if [ ! -e data/fbank/.musan.done ]; then
    ./local/compute_fbank_musan.py --num-mel-bins $num_mel_bins
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  if [ ! -e $fbank_dir/audioset_cuts_full.jsonl.gz ]; then
    lhotse combine $fbank_dir/audioset_cuts_balanced.jsonl.gz $fbank_dir/audioset_cuts_unbalanced.jsonl.gz $fbank_dir/audioset_cuts_full.jsonl.gz
  fi
  python ./local/compute_weight.py \
    --input-manifest $fbank_dir/audioset_cuts_full.jsonl.gz \
