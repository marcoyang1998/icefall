...
class PromptedAudioEncoder(nn.Module):
    ...
    def get_average_task_embed(self):
        device = next(self.parameters()).device
        task_ids = torch.tensor([i for i in range(1, self.num_tasks+1)]).to(device)
        task_embed = self.soft_prompt_embed(task_ids) # (num_tasks, C)
        avg_task_embed = task_embed.mean(dim=0) # (C)
        
        return avg_task_embed
    
    def forward_task_id(
        self, task_ids, return_average_task_embed=False,
    ):
        ...
        if return_average_task_embed:
            device = next(self.parameters()).device
            dummy_task_ids = torch.tensor([i for i in range(1, self.num_tasks+1)]).to(device)
            dummy_task_embed = self.soft_prompt_embed(dummy_task_ids) # (num_tasks, C)
            dummy_task_embed = dummy_task_embed.reshape(-1, self.soft_prompt_len, self.soft_prompt_dim)
            dummy_task_embed = self.prompt_proj(dummy_task_embed) # (num_tasks, soft_prompt_len, encoder_dim)
            dummy_task_embed = dummy_task_embed.mean(dim=0).unsqueeze(0) # (1, soft_prompt_len, encoder_dim)
            dummy_task_embed = dummy_task_embed.repeat(N,1,1) # (N, soft_prompt_len, encoder_dim)
            soft_prompt = dummy_task_embed.permute(1,0,2)
        else:    
            soft_prompt = self.soft_prompt_embed(task_ids) # (N,C)
    ...
    def forward_encoder(
        ...
        if self.use_soft_prompt and task_ids is not None:
            soft_prompt = self.forward_task_id(task_ids, return_average_task_embed=use_avg_task_emb) # (N, soft_prompt_len, encoder_dim)
        else:
            soft_prompt = None
