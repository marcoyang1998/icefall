...
from train_ffw_ada3 import add_model_arguments, get_model, get_params
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If true, use universal prompt, i.e setting all task_ids to 0"
    )
...
def main():
    ...
    analyze_ada_weights(model, params)
...
def analyze_ada_weights(model, params):
    ds_factors = _to_int_tuple(params.downsampling_factor)
    for i, encoder in enumerate(model.encoder.encoders):
        if ds_factors[i] != 1:
            encoder = encoder.encoder
        for j, layer in enumerate(encoder.layers):
            ada_weight = layer.feed_forward2.history_weight
            logging.info(f"Encoder block {i}, Layer {j}: {ada_weight[0]}")
