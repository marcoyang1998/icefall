...
from model3 import PromptedAudioEncoder
...
from zipformer_ffw_ada3 import Zipformer2
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of adapters parallel",
    )
    ...
    parser.add_argument(
        "--learnable-uni-weight",
        type=str2bool,
        default=False,
        help="If using a learnable weight for the universal mode"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Starting from which layer shall we do soft-prompting and adapters"
    )
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, audio_tagging_loss, sv_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
    ...
    if params.do_speaker_verification:
        info["speaker_verification_loss"] = sv_loss.detach().cpu().item()
...
def run(rank, world_size, args):
    ...
    if params.bucketing_sampler:
        ...
        if len(train_cuts) > 1:
            ...
            logging.info(f"Training cuts: {train_cuts_lens}")
