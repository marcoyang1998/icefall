...
from train_ada_hard import add_model_arguments, get_model, get_params
...
def analyze_ada_weights(model, params):
    ...
    for i, encoder in enumerate(model.encoder.encoders):
        ...
        for j, layer in enumerate(encoder.layers):
            ada_weight = layer.mid_adapter.history_weight
            logging.info(f"Encoder block {i}, Layer {j}: {ada_weight}")
