...
class Zipformer2(EncoderInterface):
    ...
    def forward(
        ...
        adapter_weight: Optional[Tensor] = None,
        ...
        if self.training and memory is not None:
            ...
            memory_dropout_mask = torch.rand(batch_size, 1, device=memory.device) > self.memory_dropout_rate
            memory = memory * memory_dropout_mask
            # adapter_weight[~(memory_dropout_mask.squeeze(-1))] = self.universal_ada_weights.softmax(-1)
            if random.random() < 0.02:
                logging.info(f"Adapter weights: {adapter_weight})")
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.use_adapters = use_adapters
        if use_adapters:
            # This configuration of MultiAdapter receives a mask for adapter hard selection
            # The unused adapter will have 0
            self.num_tasks = num_tasks
            ...
            self.universal_ada_weights = nn.Parameter(torch.full((self.num_tasks,), 0.5))
        else:
    ...
    def forward(
        ...
        if self.use_adapters and cross_attn is not None:
            ...
            memory_dropout_mask = memory[0,:, 0] == 0.0 # (N,)
            universal_ada_weights = self.universal_ada_weights.softmax(dim=-1)
            universal_ada_weights = universal_ada_weights.unsqueeze(0).repeat(N,1) # (N, num_tasks)
            universal_ada_weights = universal_ada_weights * memory_dropout_mask.unsqueeze(-1)
            
            adapter_weight[memory_dropout_mask] = 0.0
            adapter_weight = adapter_weight + universal_ada_weights
            if random.random() < 0.005 and not self.training:
                logging.info(f"Universal adapter weights: {universal_ada_weights}")
        else:
            adapter_weight = None
            L,N, _ = src.shape
            universal_ada_weights = self.universal_ada_weights.softmax(dim=-1)
            adapter_weight = universal_ada_weights.unsqueeze(0).repeat(N,1) # (N, num_tasks)
...
class MultiAdapter(nn.Module):
    ...
    def forward(self, x, weight=None):
        ...
        if not self.training:
            self.history_weight = self.history_weight * self.history_count + weight.sum(dim=0)
