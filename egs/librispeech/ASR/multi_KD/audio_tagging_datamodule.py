# Copyright      2023  Xiaomi Corporation     (Author: Xiaoyu Yang)
#
...
from dataset import AudioTaggingDataset
...
class AudiosetDataModule:
    """
    DataModule for audio tagging experiments.
    It assumes there is always one train and valid dataloader,
    but there can be multiple test dataloaders (e.g. Audioset balanced
    and unbalanced).
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group(
            title="Audio tagging related options",
        ...
        group.add_argument(
            "--manifest-dir",
            type=Path,
            default=Path("data/fbank_audioset"),
            help="Path to directory with audioset cuts.",
        )
        group.add_argument(
            "--max-duration",
            type=int,
    ...
    def audioset_train_cuts(self) -> CutSet:
        logging.info(f"About to get cuts from audioset {self.args.subset}")
        return load_manifest_lazy(
            self.args.manifest_dir / f"cuts_audioset_{self.args.subset}.jsonl.gz"
        )
