...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale for speaker verification loss.",
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="If use task ID when doing pre-training"
    )
...
def compute_loss(
    ...
    # speaker labels
    spkr_label = [c.supervisions[0].speaker if hasattr(c.supervisions[0], "speaker") else -1 for c in cuts]
    spkr_label = [params.spkr_dict.get(id, 0) for id in spkr_label]
    spkr_label = torch.tensor(spkr_label).to(device)
    
    feature_lens = supervisions["num_frames"].to(device)

    # audio tagging label
    audio_tagging_label = torch.tensor([0 for _ in cuts]).to(device) # the label indices are in CED format
    ...
    # 0 for ASR, 1 for SV, 2 for audio tagging
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if params.do_sv:
            loss += params.sv_loss_scale * sv_loss
    ...
    if params.do_sv:
        info["sv_loss"] = sv_loss.detach().cpu().item()
...
def run(rank, world_size, args):
    ...
    params.num_spkrs = num_speakers[params.voxceleb_subset]
    ...
    if params.use_voxceleb and params.voxceleb_subset not in ["train-all-shuf", "train-clean-100"]:
    ...
    if params.voxceleb_subset not in ["train-all-shuf", "train-clean-100"]:
        ...
        params.spkr_dict = librispeech.librispeech_train_all_shuf_spkr_dict()
