...
def decode_one_batch(
    ...
    # beats
    beats_logits = model.forward_beats(encoder_out, encoder_out_lens)
    logging.info(beats_logits.topk(5))
    beats_prediction = beats_logits.argmax(dim=-1).squeeze(-1)
    
    return beats_prediction
...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        beats_embeddings = batch["beats_embedding"]
        teacher_beats_labels = beats_embeddings.argmax(dim=-1).squeeze(-1) # (N,1,1)

        beats_prediction = decode_one_batch(
            params=params,
            model=model,
            batch=batch,
        )
...
def main():
    ...
    params.res_dir = params.exp_dir / "inference"
    ...
    test_other_dl = librispeech.valid_dataloaders(test_other_cuts)
    musan_dl = librispeech.valid_dataloaders(musan_cuts)
    
    test_sets = ["test-other", "musan"]
    test_dl = [test_other_dl, musan_dl]
    
    test_sets = ["musan"]
    test_dl = [musan_dl]
    
    beats_class_dict = get_class_dict()
    ...
    for test_set, test_dl in zip(test_sets, test_dl):
        results_dict = decode_dataset(
            ...
            params=params,
            model=model,
            beats_class_dict=beats_class_dict,
        )
