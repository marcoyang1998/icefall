...
def main():
    ...
    dirs = os.listdir(root_dir)[1:]
    for spkr in dirs:
        logging.info(f"------------For speaker {spkr}---------------")
        
        sound_files = glob.glob(f'{root_dir}/{spkr}/*/*.flac')
        ...
        for i in range(len(waves)):
            for j in range(len(waves)):
                sim, prediction = similarity(ecapa_embeddings[i], ecapa_embeddings[j])
                logging.info(f"Sample {i} to {j}: {sim}, decision: {prediction}")
    
    # sim, prediction = similarity(ecapa_embeddings[0], ecapa_embeddings[1])
    # logging.info(sim, prediction)
    # sim, prediction = similarity(ecapa_embeddings[0], ecapa_embeddings[2])
    # logging.info(sim, prediction) 
