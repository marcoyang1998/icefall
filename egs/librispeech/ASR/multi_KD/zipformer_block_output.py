...
class Zipformer2(EncoderInterface):
    ...
    def __init__(
        ...
        # weight to combine the output of each block
        self.weights = nn.Parameter(torch.full((len(self.encoder_dim),), 1/len(self.encoder_dim)))
    ...
    def forward(
        ...
        x = self._combine_output_weighted(outputs)
    ...
    def _combine_output_weighted(self, outputs: List[Tensor]):
        T, N, _ = outputs[-1].shape
        weight = self.weights.softmax(-1) # normalize the weight
        output_dim = max(self.encoder_dim)
        weighted_output = torch.zeros((T,N,output_dim), device=weight.device)
        for i in range(len(outputs)):
            cur_output = outputs[i]
            cur_d = self.encoder_dim[i]
            weighted_output[..., :cur_d] += cur_output * weight[i]
        if random.random() > 0.98:
            logging.info(f"The weight for each block is {self.weights.softmax(-1)}")
