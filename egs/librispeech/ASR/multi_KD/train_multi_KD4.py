...
def compute_loss(
    ...
    with warnings.catch_warnings():
        ...
        beats_embeddings = extract_beats_embeddings(cuts_pre_mixed).to(device) # (N,1,527)
        ecapa_embeddings = extract_ecapa_embeddings(cuts_pre_mixed).to(device) # (N, 192)
        whisper_embeddings, whisper_embedding_lens = extract_whisper_embeddings(cuts_pre_mixed)
...
def extract_ecapa_embeddings(cuts):
    ecapa_embeddings = collate_custom_field(
        cuts, "ecapa_embedding", pad_value=-100
    ) # (N,C)
    return ecapa_embeddings

def extract_whisper_embeddings(cuts):
    whisper_embeddings, whisper_embedding_lens = collate_custom_field(
        cuts, "whisper_embedding", pad_value=-100
    ) # (N,T,C)
...
def run(rank, world_size, args):
    ...
    train_cuts = train_cuts.map(add_dummy_text) # add dummy text for non-ASR data
