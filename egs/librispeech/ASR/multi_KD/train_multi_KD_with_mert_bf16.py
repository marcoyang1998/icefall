...
from model_shift_mert import MultiKDModel
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="The dimension of the mert features",
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many times to repeat fma",
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = MultiKDModel(
        ...
        use_mert=params.use_mert,
        mert_dim=params.mert_dim,
...
def compute_loss(
    ...
    mert_embeddings = batch["mert_embedding"]
    mert_embedding_lens = batch["mert_embedding_lens"]
    whisper_codebook_indexes = None
    whisper_codebook_indexes_lens = None
    
    if params.use_autocast == False and params.dtype == torch.bfloat16:
        feature = feature.to(params.dtype)
        beats_embeddings = beats_embeddings.to(params.dtype)
        ecapa_embeddings = ecapa_embeddings.to(params.dtype)
        whisper_embeddings = whisper_embeddings.to(params.dtype)
        mert_embeddings = mert_embeddings.to(params.dtype)
    ...
    with torch.set_grad_enabled(is_training):
        beats_loss, ecapa_loss, whisper_loss, mert_loss, whisper_cb_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=None,
            teacher_beats_embeddings=beats_embeddings.to(device) if params.use_beats else None,
            teacher_ecapa_embeddings=ecapa_embeddings.to(device) if params.use_ecapa else None,
            ...
            teacher_mert_embeddings=mert_embeddings.to(device) if params.use_mert else None,
            teacher_mert_embedding_lens=mert_embedding_lens.to(device) if params.use_mert else None,
        ...
        if params.use_mert:
            if task_id is not None:
                mert_mask = task_id == 3 #  music data has ID of 3
                mert_loss.clamp_(-30, 30) # loss clamping
                mert_loss *= mert_mask.unsqueeze(-1).unsqueeze(-1)
            mert_loss = mert_loss.sum()
            loss += mert_loss * params.mert_loss_scale
    ...
    if whisper_loss is not None:
        info["mert_loss"] = mert_loss.detach().cpu().item()
...
def run(rank, world_size, args):
    ...
    logging.info(f"Using mux to combine Librispeech: {params.use_librispeech}," +
        f"wenetSpeech: {params.use_wenetspeech}, audioset: {params.use_audioset}, voxceleb: {params.use_voxceleb}, and fma:{params.use_fma}"
    )
    if len(train_cuts) > 1:
    ...
    def remove_short_and_long_utt(c: Cut):
        if c.duration < 1.0 or c.duration > 30.1:
            return False
