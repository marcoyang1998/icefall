...
from ecapa_tdnn import AttentiveStatisticsPooling
...
class MultiKDModel(nn.Module):
    def __init__(
        ...
        if use_mert:
            self.mert_projection = nn.Linear(encoder_dim, 3 * mert_dim) # a linear transform
        else:
            self.mert_projection = None
        
        self.delta_t = delta_t
        if delta_t > 0:
            logging.info(f"Delta_t: {delta_t} when computing the distillation loss for Whisper and MERT")
    ...
    def forward(
        ...
        teacher_mert_embeddings: torch.Tensor = None,
        teacher_mert_embedding_lens: torch.Tensor = None,
        ...
        if self.use_whisper and teacher_whisper_embeddings is not None:
            ...
            teacher_whisper_embeddings = self.concat_successive_teacher_embeddings(
                whisper_embeddings,
                teacher_whisper_embeddings,
                k=2,
            )
        ...
        # 4. mert loss
        if self.use_mert and teacher_mert_embeddings is not None:
            mert_embeddings = self.forward_mert(encoder_out, encoder_out_lens)
            teacher_mert_embeddings = self.concat_successive_teacher_embeddings(
                mert_embeddings,
                teacher_mert_embeddings,
                k=3, # MERT teacher is 75 Hz
            )
            
            mask = make_pad_mask(encoder_out_lens)
            if self.delta_t > 0:
                mert_embeddings = mert_embeddings[:, self.delta_t:, :] 
                teacher_mert_embeddings = teacher_mert_embeddings[:, :-self.delta_t, :]
                mask = mask[:, self.delta_t:]
                
            mert_loss = F.l1_loss(mert_embeddings, teacher_mert_embeddings, reduction="none")
            mert_loss.masked_fill_(mask.unsqueeze(-1), 0.0)
            
            if reduction == "sum":
                mert_loss = mert_loss.sum() / teacher_mert_embeddings.shape[-1]
            else:
                mert_loss = mert_loss / teacher_mert_embeddings.shape[-1]
        else:
            mert_loss = None
        ...
        return beats_loss, ecapa_loss, whisper_loss, mert_loss, whisper_cb_loss
    ...
    def forward_mert(
        ...
        return self.mert_projection(F.relu(encoder_out))
    ...
    @staticmethod
    def concat_successive_teacher_embeddings(encoder_out, teacher_embeddings, k: int = 2):
        # concat every k teacher embeddings so that the frame rate is the same as the student
        # k is the time ratio
        
        t_expected = encoder_out.shape[1]
        N, T, C = teacher_embeddings.shape
        
        if T >= t_expected * k:
            teacher_embeddings = teacher_embeddings[:, : t_expected * k, :]
            teacher_embeddings = teacher_embeddings.reshape(N, t_expected, C * k)
        else:
            teacher_embeddings = teacher_embeddings[:, : t_expected, :]

        # An ugly way to avoid shape mismatch of dummy embedding, this only happens if the whole batch is dummy embedding!
        if teacher_embeddings.shape[1] != encoder_out.shape[1]:
            return encoder_out

        assert teacher_embeddings.shape[1] == encoder_out.shape[1]
        assert teacher_embeddings.shape[2] == encoder_out.shape[2], f"{N}x{T}x{C} {teacher_embeddings.shape}, {encoder_out.shape}"
