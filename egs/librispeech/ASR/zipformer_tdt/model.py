...
class AsrModel(nn.Module):
    def __init__(
        ...
        if use_transducer:
            ...
            self.rnnt_loss = TDTLossNumba(
                blank=0, # in icefall, the default blank ID is 0
                reduction="sum",
                durations=durations,
                sigma=tdt_sigma,
                omega=tdt_omega,
    ...
    def forward_transducer(
        ...
        encoder_out = self.joiner.encoder_proj(encoder_out).unsqueeze(2)
        decoder_out = self.joiner.decoder_proj(decoder_out).unsqueeze(1)
    ...
    def forward(
        ...
        if self.use_transducer:
            # Compute transducer loss
            tdt_loss = self.forward_transducer(
                encoder_out=encoder_out,
                encoder_out_lens=encoder_out_lens,
                y=y.to(x.device),
                y_lens=y_lens.to(x.device),
                prune_range=prune_range,
                am_scale=am_scale,
                lm_scale=lm_scale,
            )
        else:
            tdt_loss = torch.empty(0)
