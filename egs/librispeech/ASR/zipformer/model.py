...
class MultiLayerKDAsrModel(nn.Module):
    def __init__(
        self,
        encoder_embed: nn.Module,
        encoder: EncoderInterface,
        ...
        Args:
          ...
          kd_dim:
            The dimension of teacher embeddings
          cb_input_dims:
            The input dimension to the embedding loss module. In the form of a list.
        """
        super().__init__()
        ...
        embedding_loss_modules = []
        if teacher_embedding_dim > 0:
            for i, input_dim in enumerate(cb_input_dims):
                embedding_loss_net = nn.Linear(input, teacher_embedding_dim)
                embedding_loss_modules.append(embedding_loss_net)
            self.embedding_loss_modules = nn.ModuleList(embedding_loss_modules)
    ...
    def forward(
        ...
        lm_scale: float = 0.0,
        codebook_indexes: torch.Tensor = None,
        teacher_embeddings: List[torch.Tensor] = None,
        teacher_embedding_lens: torch.Tensor = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Args:
        ...
        if self.training and hasattr(self, "embedding_loss_modules"):
            middle_out = middle_out[0]
            assert self.embedding_loss_net is not None
            assert len(middle_out) == len(self.embedding_loss_modules), "Must have the same amount"

            embedding_loss = 0.0
            for loss_net, student_embedding, teacher_embedding in zip(self.embedding_loss_modules, middle_out, teacher_embeddings):
                projected_embeddings = self.embedding_loss_net(student_embedding) # (N,T,C)
                embedding_loss = self.forward_embedding_loss(
    ...
    def forward_embedding_loss(
        projected_embeddings: torch.Tensor,
        teacher_embeddings: torch.Tensor,
        encoder_out_lens: torch.Tensor,
    ):
        # Compute the l1 embedding loss between the projected student embedding and
        # the teacher embeddings
        
        assert teacher_embeddings.size(1) >= projected_embeddings.size(1)
        expected_T = projected_embeddings.size(1)
        teacher_embeddings = teacher_embeddings[:,:expected_T,:] # truncate the teacher embeddings
        assert teacher_embeddings.shape == projected_embeddings.shape
