...
from replace_dead_units import replace_dead_units
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="How often we do the pruning to the model",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="If greater than 0, prune the channel according to the normalized"
        "channel weights. The pruned channel weights will be replaced by more"
        "importance channels"
    )
...
def run(rank, world_size, args):
    ...
    if params.channel_pruning_threshold > 0.0:
        logging.info(f"Try replacing the less important channels")
        model_state_dict = model.state_dict()
        model_state_dict = replace_dead_units(model_state_dict=model_state_dict, threshold=params.channel_pruning_threshold)
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        # Replace dead channels with more important ones
        if params.channel_pruning_threshold > 0.0 and epoch % params.pruning_period == 0:
