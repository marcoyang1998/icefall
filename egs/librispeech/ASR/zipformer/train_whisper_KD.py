...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Whether to enable embedding distillation.",
    )
    
    parser.add_argument(
        "--teacher-embedding-dim",
        type=int,
        default=768,
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="The scale of embedding loss.",
    )
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    encoder = Zipformer2(
        ...
        if params.embedding_distillation
        else None,
    )
    return encoder
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = AsrModel(
        ...
        teacher_embedding_dim=params.teacher_embedding_dim if params.embedding_distillation else 0,
        num_codebooks=0,
...
def extract_whisper_embedding(batch: Dict, key="whisper_embedding") -> Tuple[Tensor, Tensor]:
    ...
    return whisper_embeddings, whisper_embedding_lens
...
def compute_loss(
    ...
    if is_training:    
        whisper_embeddings, whisper_embedding_lens = extract_whisper_embedding(batch)
        whisper_embeddings = whisper_embeddings.to(device)
        whisper_embedding_lens = whisper_embedding_lens.to(device)
    else:
        whisper_embeddings, whisper_embedding_lens = None, None

    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, ctc_loss, codebook_loss, embedding_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
            ...
            teacher_embeddings=whisper_embeddings,
            teacher_embedding_lens=whisper_embedding_lens,
        )
        ...
        if is_training and params.embedding_distillation:
            loss += params.embedding_loss_scale * embedding_loss
    ...
    if is_training and params.embedding_distillation:
        info["embedding_loss"] = embedding_loss.detach().cpu().item()
