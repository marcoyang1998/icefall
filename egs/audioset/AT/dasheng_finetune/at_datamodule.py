...
from dataset import WaveformAudioTaggingDataset
...
class AudioSetATDatamodule:
    ...
    def waveform_train_dataloaders(self, 
        cuts_train: CutSet,
        sampler_state_dict: Optional[Dict[str, Any]] = None
    ) -> DataLoader:
        train = WaveformAudioTaggingDataset(
            collate=True,
        )
        if self.args.bucketing_sampler:
            assert (
    ...
    def waveform_valid_dataloaders(self, cuts_valid: CutSet) -> DataLoader:
        validate = WaveformAudioTaggingDataset(collate=True)
        valid_sampler = DynamicBucketingSampler(
            cuts_valid,
            max_duration=self.args.max_duration,
            shuffle=False,
    ...
    def waveform_test_dataloaders(self, cuts: CutSet) -> DataLoader:
        logging.debug("About to create test dataset")
        test = WaveformAudioTaggingDataset(collate=True)
        sampler = DynamicBucketingSampler(
            cuts,
            max_duration=self.args.max_duration,
            shuffle=False,
