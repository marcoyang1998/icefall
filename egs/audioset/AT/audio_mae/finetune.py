...
def get_parser():
    ...
    parser.add_argument(
        "--lr", type=float, default=0.0045, help="The base learning rate."
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = VisionTransformer(
        ...
        mask_2d=params.mask_2d,
        use_custom_patch=params.use_custom_patch,
        num_classes=params.num_events,
        norm_layer=nn.LayerNorm,
    )

    img_size = (1024, 128)

    if params.use_custom_patch:
        logging.info(f"Using custom patch")
        model.patch_embed = PatchEmbed_new(img_size=img_size, patch_size=16, in_chans=1, embed_dim=emb_dim, stride=10)
        model.pos_embed = nn.Parameter(torch.zeros(1, 1212 + 1, params.encoder_dim), requires_grad=False)  # fixed sin-cos embedding
    else:
        ...
        model.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, params.encoder_dim), requires_grad=False)  # fixed sin-cos embedding
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            ...
            cur_lr = lr_sched.adjust_learning_rate(optimizer, batch_idx/475 + params.cur_epoch, params)
...
def run(rank, world_size, args):
    ...
    if params.do_finetune:
        ...
        checkpoints = torch.load(params.finetune_ckpt, map_location='cpu')["model"]
        state_dict = model.state_dict()
        for k in state_dict.keys():
            if k in checkpoints:
                state_dict[k] = checkpoints[k]
            else:
                logging.info(f"Didn't find match for {k}")
