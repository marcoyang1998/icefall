...
from train_mask_fbank_only_masked_co_training import add_model_arguments, get_model, get_params
...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        correlation_factor = inference_one_batch(
            params=params,
            model=model,
            batch=batch,
        )

        correlations.append(correlation_factor.item())
...
def main():
    ...
    params.res_dir = params.exp_dir / "analyze_correlation_factor"
    ...
    correlations = decode_dataset(
        dl=audioset_dl,
        params=params,
        model=model,
    )

    logging.info(f"Averaged correlation factor for audioset eval is: {sum(correlations)/len(correlations)}")
    logging.info("Done")
