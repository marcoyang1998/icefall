...
class AudioPretrainingModel(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            fbank = fbank.permute(1,0,2) # (N,T,C) -> (T,N,C)
        ...
        decoder_in = decoder_in.permute(1,0,2) # (N,T,C)
        T_decoder = decoder_in.size(1)

        # repeat the last frame if T is uneven
        if T_decoder % 2 == 1:
            decoder_in = torch.cat([decoder_in, decoder_in[:,-1:, :]], dim=1)
            decoder_src_key_padding_mask = torch.cat(
                [
                    decoder_src_key_padding_mask,
                    torch.ones([N, 1], device=x.device).bool(),
                ],
                dim=1
            )
        
        decoder_in = decoder_in.chunk(2, dim=1) # ((N, T//2, C), (N, T//2, C))
        decoder_in = torch.cat(decoder_in, dim=0) # (2*N, T//2, C)
            
        decoder_src_key_padding_mask = decoder_src_key_padding_mask.chunk(2, dim=1)
        decoder_src_key_padding_mask = torch.cat(decoder_src_key_padding_mask, dim=0)

        decoder_in_lens = decoder_src_key_padding_mask.size(1) - decoder_src_key_padding_mask.sum(dim=1).int() # (2*N, )

        decoder_in = decoder_in.permute(1,0,2)
        # (2*N, T//2, C)
        decoder_out, decoder_out_lens = self.decoder(decoder_in, decoder_in_lens, decoder_src_key_padding_mask)
        decoder_out = decoder_out.permute(1, 0, 2) # (T//2, 2*N, C) -> (2*N, T//2, C)

        decoder_out = torch.cat([decoder_out[:N, ], decoder_out[N:, ]], dim=1) # (N, T, C)
        # delete the repeated frame if necessary
        if T_decoder % 2 == 1:
            decoder_out = decoder_out[:,:-1]
        assert decoder_out.size(1) == T_decoder
