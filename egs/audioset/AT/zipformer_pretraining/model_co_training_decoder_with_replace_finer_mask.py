...
class AudioPretrainingModel(nn.Module):
    def __init__(
        ...
        # co-training proj
        self.co_training_proj = nn.Linear(decoder_dim, encoder_dim, bias=False)

        # fbank proj for replace decoder_in with fbank
        self.fbank_proj = nn.Linear(fbank_dim, decoder_input_dim // 4, bias=False)
    ...
    def forward(
        ...
        fbank = x.clone() # get a copy of input fbank
        padding_mask = make_pad_mask(x_lens)
        target = target.repeat(2,1,1)

        # apply masking to two copies of fbank features
        x1, mask_indices1 = self.apply_mask_facebook(
            x.clone(),
            padding_mask=padding_mask
        ) # (N,T,C), (N,T)

        x2, mask_indices2 = self.apply_mask_facebook(
            x.clone(),
            padding_mask=padding_mask
        ) # (N,T,C), (N,T)
        ...
        encoder_out_orig = encoder_out.clone()
        ...
        mask_indices = torch.cat([mask_indices1, mask_indices2], dim=0) # (2 * N， T)
        ...
        if self.training:
            fbank = fbank.repeat(2,1,1) # duplicate the fbank
            fbank = self.fbank_proj(fbank)
            fbank = fbank[:, :decoder_in.size(0)*4, :]
            fbank = fbank.reshape(2*N, decoder_in.size(0), decoder_in.size(-1))
            fbank = fbank.permute(1,0,2) # (T,N,C) -> (N,T,C)
            non_masked_indices = mask_indices == False 
            replace_mask = torch.rand(mask_indices.shape, device=x.device) <= self.fbank_replace_prob
        ...
        if self.training:
            co_training_loss = self.forward_co_training_loss(
                ...
                encoder_out=encoder_out_orig,  # use the original encoder_out as target
    ...
    def forward_co_training_loss(
        ...
        """Compute the co-training loss. This is achieved by projecting the decoder_out
        to encoder_dim. Note we only compute the loss on the positions where
        the source is masked & the target is not masked

        Args:
            decoder_out (torch.Tensor): The output of the decoder, i.e. the source, (T,N,C)
            encoder_out (torch.Tensor): The output of the encoder, i.e. the target, (T,N,C)
            encoder_out_lens (torch.Tensor): The length of sequence
            mask_indices1 (torch.Tensor): The mask indices of the first batch
            mask_indices2 (torch.Tensor): The mask indices of the second batch
        ...
        pred_encoder_out = self.co_training_proj(decoder_out)
        ...
        # downsample the mask
        valid_co_training_mask = downsample_mask_indices(valid_co_training_mask)
