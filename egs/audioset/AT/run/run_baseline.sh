...
python zipformer/train_KD.py \
    --world-size 4 \
    --num-epochs 150 \
    --exp-dir zipformer/exp_at_${subset}_lr_epochs_${lr_epochs}_specaug${enable_spec_aug}_frame${frames_mask_size}_feature${features_mask_size}_musan${enable_musan}_weighted${weightd_sampler}_md${md} \
    --start-epoch 1 \
    --use-fp16 1 \
    --num-events 527 \
