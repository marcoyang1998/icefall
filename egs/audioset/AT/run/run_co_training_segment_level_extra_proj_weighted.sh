...
python zipformer_co_training/train_mtl_segment_level_extra_proj.py \
    --world-size 4 \
    --num-epochs 100 \
    --exp-dir zipformer_co_training/exp_at_as_${subset}_co_training_extra_proj_segment_level_segment_length_${segment_length}_scale${co_training_scale}_segment_scale${segment_level_co_training_loss_scale}_aux_scale${segment_level_at_loss_scale}_matched_specaug_num_masks${num_frame_masks}_mask_span${frames_mask_size}_lr_epochs${lr_epochs}_weighted_sampling${weightd_sampler}_md${md} \
    --start-epoch 1 \
    --use-fp16 1 \
    --num-events 527 \
