...
class AudioPretrainingModel(nn.Module):
    ...
    def forward(
        ...
        nan_mask = l2_loss == torch.nan
        l2_loss[nan_mask] = 0.0
...
if __name__=="__main__":
    ...
    for mask_prob in [0.5, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
        proportions = []
        for i in range(50):
            ...
            x = torch.rand(5, 405, 80)
            x_lens = torch.tensor([405, 401, 400, 400, 394])
            padding_mask = make_pad_mask(x_lens)
            x_masked, mask_indices = model.apply_mask_custom(x, padding_mask=padding_mask)
