...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    if params.frame_level_co_training:
        from model_mtl_co_training_with_mask import AudioTaggingModel
        logging.info("Doing bi-level co-training")
    else:
        from model import AudioTaggingModel
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        at_loss, frame_level_co_training_loss, clip_level_co_training_loss = model(
            x=feature,
            x_lens=feature_lens,
            target=labels,
        ...
        loss = (
            ...
            + frame_level_co_training_loss * co_training_loss_scale * 0.5
            + clip_level_co_training_loss * co_training_loss_scale * 0.5
        )
    ...
    if is_training:
        info["frame_level_co_training_loss"] = frame_level_co_training_loss.detach().cpu().item()
