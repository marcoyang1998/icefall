...
class AudioTaggingModel(nn.Module):
    ...
    def forward(
        ...
        frame_logits = self.forward_audio_tagging(
        ...
        if self.training:
            # compute the frame-level co-training loss
            co_training_target = torch.cat(
                [frame_logits[N:].detach(), frame_logits[:N].detach()]
            ).sigmoid()  # exchange target
            frame_level_co_training_loss = self.criterion(frame_logits, co_training_target)  # (2 * N, T, 527)

            # mask the co-training loss at padding positions
            frame_level_co_training_loss.masked_fill_(padding_mask.unsqueeze(-1), 0)
            ...
            valid_co_training_mask = valid_co_training_mask[:, :frame_level_co_training_loss.size(1)]
            frame_level_co_training_loss *= valid_co_training_mask.unsqueeze(dim=-1)
        
            frame_level_co_training_loss = frame_level_co_training_loss.sum() / (valid_co_training_mask.sum() / (2*N))
            if random.random() < 0.1:
                logging.info(f"The number of valid co_training frames: {valid_co_training_mask.sum()}")
        else:
            frame_level_co_training_loss = 0.0
        ...
        frame_level_at_loss = self.criterion(frame_logits, target.unsqueeze(dim=1).repeat(2, frame_logits.size(1), 1)) # (2 * N, T, 527)
        frame_level_at_loss = frame_level_at_loss.masked_fill(padding_mask.unsqueeze(-1), 0.0).sum() / frame_logits.size(1)

        return loss / 2.0, frame_level_at_loss / 2.0, frame_level_co_training_loss / 2.0, clip_level_co_training_loss
