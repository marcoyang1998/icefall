...
class AudioTaggingModel(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            frame_level_co_training_target = torch.cat(
                [logits[N:].detach(), logits[:N].detach()]
            ).sigmoid()  # exchange target
            frame_level_co_training_loss = self.criterion(logits, frame_level_co_training_target)  # (2 * N, T, 527)
            ...
            frame_level_co_training_loss = frame_level_co_training_loss.sum() / (valid_co_training_mask.sum() / N)
        ...
        logits = logits / (~padding_mask).sum(dim=1).unsqueeze(-1).expand_as(logits) # (2*N, num_classes)
        ...
        return loss / 2.0, frame_level_co_training_loss / 2.0, clip_level_co_training_loss / 2.0
