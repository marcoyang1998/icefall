...
from model_mtl_co_training_with_mask_extra_proj import AudioTaggingModel
...
def get_parser():
    ...
    parser.add_argument(
        "--frame-level-at-loss-scale",
        type=float,
        default=0.1,
        help="The loss scale for the frame-level AT loss",
    )
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        at_loss, frame_level_at_loss, frame_level_co_training_loss, clip_level_co_training_loss  = model(
            x=feature,
            x_lens=feature_lens,
            target=labels,
        ...
        loss = (
            ...
            + frame_level_at_loss * params.frame_level_at_loss_scale * (1 - co_training_loss_scale)
            + frame_level_co_training_loss * params.frame_level_co_training_scale * co_training_loss_scale
            + clip_level_co_training_loss * (1- params.frame_level_co_training_scale) * co_training_loss_scale
        )
    ...
    info["frame_level_at_loss"] = frame_level_at_loss.detach().cpu().item()
    if is_training:
        info["frame_level_co_training_loss"] = frame_level_at_loss.detach().cpu().item()
