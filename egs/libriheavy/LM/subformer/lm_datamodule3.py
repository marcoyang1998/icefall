...
class LmDataset(torch.utils.data.IterableDataset):
    def __init__(self,
        ...
        self.bytes_per_segment = int(tokens_per_segment * 1.6) # this is an empirical value
        ...
        self.num_segments = float('inf') if training else 1 + tot_positions // (self.bytes_per_segment * tot_workers)
    ...
    def __iter__(self):
        ...
        while n < self.num_segments:  # if self.num_segments is infinity, just keep going.
            ...
            if len(tokens) < self.tokens_per_segment:
                tokens = tokens + [0] * (self.tokens_per_segment - len(tokens))
            if len(tokens) > self.tokens_per_segment:
                tokens = tokens[:self.tokens_per_segment]
            assert len(tokens) == self.tokens_per_segment, len(tokens)
            yield torch.Tensor(tokens).to(torch.long)
