...
def load_backbone_ASR(params: AttributeDict, model: nn.Module) -> nn.Module:
    """Load the model parameters from a well-trained ASR model
...
def set_params_training_mode(model: nn.Module, update_modules: List[str]):
    # Set other modules to eval() mode except the modules matching the `update_modules`
    if len(update_modules) == 0:
        return
...
def train_one_epoch(
    ...
    set_params_training_mode(model, update_modules=params.update_modules)
