...
from model_with_llama_with_style import PromptedTransducer
...
def get_upper_only_alpha_with_context_list(
    ...
    if context_list != "":
        if random.random() < 0.5:
            # correct + distractors
            # sample distractors
            num_distractors = random.randint(0, 80)
...
def get_text_encoder(params: AttributeDict) -> nn.Module:
    ...
    if params.text_encoder_type == "llama-7b":
        from transformers import LlamaModel
        # This is a Llama-7b model
        logging.info("Loading pre-trained LLama-7B as text encoder")
        model = LlamaModel.from_pretrained(
            "meta-llama/Llama-2-7b-hf",
            torch_dtype=torch.float16,
        )
        params.text_encoder_dim = 4096
    elif params.text_encoder_type == "llama-3b":
        from transformers import LlamaModel
        # This is a Llama-3b model
        logging.info("Loading pre-trained Llama-3B as text encoder")
        model = LlamaModel.from_pretrained(
            "openlm-research/open_llama_3b_v2",
            torch_dtype=torch.float16,
        )
        params.text_encoder_dim = 3200
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        ...
        vocab_size=params.vocab_size,
        context_dim=4 * 4096 if params.context_injection else -1, # the output dim of text encoder
...
def _encode_texts_as_bytes_with_tokenizer(
    ...
    combined_text = [style_texts[i] + ' <s> ' + truncated_pre_texts[i] for i in range(batch_size)]
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        simple_loss_scale = (
            s
            if (batch_idx_train // params.accum_grad + 1) >= warm_step
            else 1.0 - ((batch_idx_train // params.accum_grad + 1) / warm_step) * (1.0 - s)
        )
        pruned_loss_scale = (
            1.0
            if (batch_idx_train // params.accum_grad + 1) >= warm_step
            else 0.1 + 0.9 * ((batch_idx_train // params.accum_grad + 1) / warm_step)
        )
        if (batch_idx_train // params.accum_grad) % params.log_interval == 1:
            logging.info(f"Simple_loss_scal: {simple_loss_scale}, pruned_loss_scale: {pruned_loss_scale}")
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.accum_grad == 0:
            params.batch_idx_train += 1
        batch_size = len(batch["supervisions"]["text"])

        try:
            with torch.cuda.amp.autocast(enabled=params.use_fp16):
                if (batch_idx + 1) % params.accum_grad != 0:
                    with model.no_sync():
                        loss, loss_info = compute_loss(
                            params=params,
                            model=model,
            ...
            if (batch_idx + 1) % params.accum_grad == 0:
                ...
                optimizer.zero_grad()
                update_count += 1
                logging.info(f"Number of updates in this epoch is {update_count}.")
        except:  # noqa
            #save_bad_model()
        ...
        if (
            ...
            and batch_idx % params.accum_grad == 0
        ):
            logging.info(f"Updating averaged model at step: {params.batch_idx_train}")
            update_averaged_model(
                params=params,
                model_cur=model,
                model_avg=model_avg,
        ...
        if (
            ...
            and batch_idx % params.accum_grad == 0 # for accum gradient
        ):
        ...
        if batch_idx % (params.accum_grad * 100) == 0 and params.use_fp16:
            ...
            if cur_grad_scale < 8.0 or (
                cur_grad_scale < 32.0 and batch_idx % (params.accum_grad * 400) == 0
            ):
        ...
        if batch_idx % (params.log_interval * params.accum_grad) == 0:
            actual_batch_idx = batch_idx // params.accum_grad
            cur_lr = max(scheduler.get_last_lr())
            cur_grad_scale = scaler._scale.item() if params.use_fp16 else 1.0

            logging.info(
                f"Epoch {params.cur_epoch}, "
                f"batch {actual_batch_idx}, loss[{loss_info}], "
...
def run(rank, world_size, args):
    ...
    text_sampling_func = triplet_text_sampling2
    ...
    valid_cuts = valid_cuts.subset(first=1500)
