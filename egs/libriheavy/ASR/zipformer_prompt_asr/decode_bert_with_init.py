...
from train_bert_encoder_init import (
    add_model_arguments,
    get_params,
    get_tokenizer,
...
def main():
    ...
    long_audio_dl = libriheavy.valid_dataloaders(
        long_audio_cuts, text_sampling_func=naive_triplet_text_sampling
    )
    npr1_dev_dl = libriheavy.valid_dataloaders(
        npr1_dev_cuts, text_sampling_func=naive_triplet_text_sampling
    )
    npr1_test_dl = libriheavy.valid_dataloaders(
        npr1_test_cuts, text_sampling_func=naive_triplet_text_sampling
    )
    ...
    for test_set, test_dl in zip(test_sets, test_dl):
        if test_set == "ls-test-clean":
            biasing_dict = get_facebook_biasing_list(
                "test-clean", use_distractors=params.ls_distractors
            )
        elif test_set == "ls-test-other":
            biasing_dict = get_facebook_biasing_list(
                "test-other", use_distractors=params.ls_distractors
            )
        else:
            biasing_dict = None
