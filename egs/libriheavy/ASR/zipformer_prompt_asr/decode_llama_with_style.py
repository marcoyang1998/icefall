...
from train_llama_encoder_with_style2 import (
    add_model_arguments,
    get_params,
    get_tokenizer,
    get_text_encoder,
...
def save_results(
    ...
    for key, results in results_dict.items():
        ...
        if params.compute_CER:
            ...
            recog_path = params.res_dir / f"recogs-{test_set_name}-{key}-char-{params.suffix}.txt"
            store_transcripts(filename=recog_path, texts=results, char_level=True)
            errs_filename = (
                params.res_dir / f"errs-CER-{test_set_name}-{key}-{params.suffix}.txt"
            )
            with open(errs_filename, "w") as f:
...
def main():
    ...
    # model = copy.deepcopy(model.text_encopder)
    ...
    # model.text_encoder = get_text_encoder(params)
    model.to(device)
    model.eval()
